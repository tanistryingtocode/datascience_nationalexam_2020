# read file
with open("clean_data_1.csv", encoding="utf8") as file:
...
for student in students:
    ...
    student_first_name = student_name[-1]
    if student_first_name not in name:
        name.append(student_first_name)
        name_count.append(0)
        name_count[name.index(student_first_name)] +=1
    else:
        name_count[name.index(student_first_name)] +=1
...
axis.set_ylim(0,3200)

# label and title
plt.ylabel('# of Students')
plt.xlabel('First Name')
plt.title('Top 10 Popular First Name among Students')
